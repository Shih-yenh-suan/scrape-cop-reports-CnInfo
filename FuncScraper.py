...
def process_page_for_downloads(pageNum):
    """处理指定页码的公告信息并下载相关文件"""
    fill_params(DATA, CATEGORY_AND_NAME, pageNum, file_type,
                cnInfoColumn, cnInfoCategory, 使用关键词而非巨潮分类)
...
def process_announcements(i):
    ...
    if any(word in file_type for word in ["社会责任", "ESG", "CSR"]):
        # 对于CSR报告，处理后缀
    ...
    # 接下来开始执行下载前的判断
    # 1. 对于标题包含停用词的报告，跳过下载
    if any(re.search(k, title) for k in get_stopwords(file_type, STOP_WORDS_DICT)):
